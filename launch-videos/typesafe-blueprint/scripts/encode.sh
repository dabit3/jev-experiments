#!/usr/bin/env bash
# Encodes out/frames/*.png -> out/typesafe-blueprint.mp4 (H.264, yuv420p, 30 fps, silent)
...
ffmpeg -y -v error -framerate "$FPS" -i out/frames/%06d.png \
  ...
  -vf "format=yuv420p" out/typesafe-blueprint.mp4
ffmpeg -y -v error -i out/typesafe-blueprint.mp4 \
...
ffprobe -v error -show_entries format=duration:stream=width,height,r_frame_rate,codec_name,pix_fmt \
  -of default=noprint_wrappers=1 out/typesafe-blueprint.mp4
ls -la out/typesafe-blueprint.mp4 out/contact-sheet.png
