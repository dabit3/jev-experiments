...
// then encodes out/swiss-grid.mp4 with ffmpeg. Usage: node render.mjs [--still 12.5]
...
execSync(`ffmpeg -y -v error -framerate ${fps} -i "${outDir}/%04d.png" -c:v libx264 -pix_fmt yuv420p -crf 16 -r ${fps} -movflags +faststart "${path.join(here, "out", "swiss-grid.mp4")}"`, { stdio: "inherit" });
execSync(`ffmpeg -y -v error -i "${path.join(here, "out", "swiss-grid.mp4")}" -vf "fps=1,scale=320:180,tile=6x5,pad=1920:1080:0:90:0x0B0B0D" "${path.join(here, "out", "contact-sheet.png")}"`, { stdio: "inherit" });
console.log("done", path.join(here, "out", "swiss-grid.mp4"));
