...
final class LiveServiceTests: XCTestCase {
  private func jev() throws -> JevClient {
    guard ProcessInfo.processInfo.environment["SAY_LIVE_TESTS"] == "1" else {
      throw XCTSkip("Set SAY_LIVE_TESTS=1 to exercise real providers.")
    }
  ...
  func testLiveActionTypesLiteralIntoFocusedEditor() async throws {
    ...
    let actions = ActionPolicy.candidates(screen: screen, apps: [:], texts: ["Hello Say"])
    struct State: Encodable, Sendable {
      var goal: String
      var screen: ScreenState
      var history: [String]
    ...
    let result = try await client.ask(
      State(goal: "Type \"Hello Say\" into this document", screen: screen, history: []),
      questions: JevClient.actionQuestions(actions))
