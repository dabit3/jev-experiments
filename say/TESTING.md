...
`AppExperienceTests` checks app-opening routes, native menu action dispatch, Quit during a task, listener dismissal, stale-error cleanup, missing-key setup, and credential-save failures. Accessibility recovery uses a fake URL opener in tests, so System Settings stays closed. The tests cover the exact destination, explicit-click behavior, and manual instructions if macOS rejects the URL. `RecoveryActionTests` checks that the recovery action survives saved history and that older messages still decode. A separate AppKit test checks the Settings toolbar, selected pane title, and window dimensions.

With Accessibility off, an action request must show an Open Accessibility Settings button in the listener and History error message. Clicking it opens macOS System Settings → Privacy & Security → Accessibility and dismisses the floating listener. Say does not grant access itself or replay the request automatically. Enable Say, return to the app, and repeat the request. The Accessibility row in Say’s Privacy pane opens the same destination.
