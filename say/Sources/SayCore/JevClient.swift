...
public struct JevClient: Sendable {
  ...
  public func ask<State: Encodable & Sendable>(
    ...
    guard !key.isEmpty else {
      throw SayError("Add your Jev API key in Settings to get started.")
    }
    ...
    for attempt in 0..<3 {
      ...
      guard Set(questions.keys).isSubset(of: Set(decoded.answers.keys)) else {
        throw SayError("Jev returned an incomplete decision. Try again.")
      }
      return JevResult(
  ...
  public static func error(_ status: Int) -> SayError {
    switch status {
    case 401, 403: SayError("Jev rejected this API key. Check it in Settings.")
    case 429, 529: SayError("Jev is busy or rate limited. Please try again shortly.")
    default: SayError("Jev could not complete the request (HTTP \(status)).")
    }
  }
  ...
  public func chooseAction<State: Encodable & Sendable>(
    ...
    guard let answer = result.answers["next"], let choice = answer.choice,
      ...
      throw SayError("Jev returned an unavailable action. I stopped without acting.")
    }
    ...
    guard (review.answers["verified"]?.noul ?? 0) >= 0.85 else {
      throw SayError("I’m not sure which control to use. Try a more specific request.")
    }
    return JevResult(
