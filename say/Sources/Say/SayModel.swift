...
final class SayModel: ObservableObject {
  ...
  var connected: Bool { !preferences.jevKey.isEmpty && !preferences.openAIKey.isEmpty }
  ...
  private func runActions(
    ...
    for step in 0..<24 {
      ...
      guard active?.bundleIdentifier != Bundle.main.bundleIdentifier else {
        throw SayError("Bring the app you want to control forward, then try again.")
      }
      ...
      if action.kind == .done {
        guard step > 0 || !goal.lowercased().contains("create") else {
          throw SayError("I couldn’t verify that a new item was created.")
        }
      ...
      guard repeats < 2 else {
        throw SayError("That control isn’t responding. I stopped rather than repeating it.")
      }
  ...
  private func execute(
    ...
    if ActionPolicy.requiresConfirmation(action, screen: screen) {
      ...
      try requireCurrent(token)
      guard approved else { throw SayError("Action cancelled. Nothing further was changed.") }
      hideWindow?()
      target?.activate()
      try await Task.sleep(for: .milliseconds(300))
      ...
      if action.kind == .press || action.kind == .focus {
        guard let old = screen.elements.first(where: { $0.id == action.value }),
          ...
          throw SayError("The control changed while waiting for approval. Please try again.")
        }
        var updated = action
