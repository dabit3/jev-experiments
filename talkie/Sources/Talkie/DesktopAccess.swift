  ...
  func perform(_ action: MacAction) async throws {
    ...
    if action.kind == .launch {
      guard let url = NSWorkspace.shared.urlForApplication(withBundleIdentifier: action.value)
        ...
        throw TalkieError("That app is no longer installed.")
      }
    ...
    if action.kind == .openURL {
      guard let url = ActionPolicy.webURL(action.value), NSWorkspace.shared.open(url) else {
        throw TalkieError("That website could not be opened.")
      }
      return
    }
    guard Self.trusted else {
      throw TalkieError("Allow Talkie in System Settings → Privacy & Security → Accessibility.")
    }
    ...
    case .press, .focus:
      guard let element = elements[action.value] else {
        throw TalkieError("That control is no longer available.")
      }
      ...
      guard let rect = frame(element), rect.width > 0, rect.height > 0 else {
        throw TalkieError("This app does not expose a usable action for that control.")
      }
    ...
    case .type:
      ...
      for character in action.value {
        try Task.checkCancellation()
        guard NSWorkspace.shared.frontmostApplication?.processIdentifier == targetPID else {
          throw TalkieError("Typing stopped because the active app changed.")
        }
    ...
    case .key:
      guard let chord = Self.chords[action.value] else {
        throw TalkieError("Unsupported keyboard shortcut.")
      }
      for down in [true, false] {
