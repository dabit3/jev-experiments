...
public struct AssistantClient: Sendable {
    ...
    -> AssistantReply
  {
    guard !key.isEmpty else {
      throw TalkieError(
        "Add an OpenAI key in Settings for conversation and web research. Jev Mac control works on its own."
      )
    }
    ...
    let body = Request(
      instructions: """
        You are Talkie, a calm, helpful Mac companion. Be concise and natural when spoken aloud.
    ...
    guard !text.isEmpty else {
      throw TalkieError("The conversation service returned no answer. Try again.")
    }
  ...
  public func transcribe(file: URL) async throws -> String {
    guard !key.isEmpty else { throw TalkieError("Add an OpenAI key to transcribe an audio file.") }
    struct Response: Decodable { var text: String }
    let audio = try Data(contentsOf: file)
    guard audio.count < 24 * 1024 * 1024 else {
      throw TalkieError("Choose an audio file smaller than 24 MB.")
    }
    ...
    guard (response as? HTTPURLResponse)?.statusCode == 200 else {
      throw TalkieError("Audio transcription failed. Check your OpenAI key and the audio format.")
    }
